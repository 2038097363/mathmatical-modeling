...
\begin{figure}[htbp]
  ...
  \caption{候选域内最低成本的已确认可行配比 $(614,6)$ 的全局轴测重建。浅灰线为 A 片段，蓝色点为 B 片段，橙色粗线为程序恢复的实际贯通见证。}
...
本参赛队在竞赛过程中使用了 WorkBuddy Agent{[}9{]}，调用模型为 DeepSeek（DS），仅用于联网搜索与资料检索，详细使用情况见附录 A。核心模型、数学推导、程序实现、参数口径、数值结果和结论均由参赛队独立完成并复核。
...
{[}9{]} WORKBUDDY. WorkBuddy Agent{[}EB/OL{]}. 2026.
...
使用智能体为 WorkBuddy Agent，调用模型为 DeepSeek（DS），使用日期为 2026-08-07 至 2026-08-08。该智能体仅用于联网搜索与资料检索，模型、推导、程序、图件、数值与结论均由参赛队独立完成并复核。
...
\bottomrule\noalign{}
\endlastfoot
资料检索 & 联网搜索有限实体接触、图连通、Monte Carlo 与二项置信区间的相关资料 & 返回网页、论文线索及内容摘要 & 参赛队打开原始来源，核对公式、适用条件和引用信息 \\
\end{longtable}
}
