...
SOURCE_LANGUAGE_BY_SUFFIX = {".py": "Python"}
...
def read_utf8_text(path: Path, role: str) -> str:
    try:
        ...
        raise ValueError(f"{role} must be valid UTF-8: {path}") from exc


def reject_private_home_paths(text: str, role: str) -> None:
    if PRIVATE_HOME_PATTERN.search(text):
        raise ValueError(f"{role} contains private home directory paths")
...
def validate_ai_header_lines(raw: Any) -> list[str]:
    if not isinstance(raw, list) or len(raw) != 2:
        raise ValueError("AI header lines must be a list of exactly two strings")
    lines = [str(line) for line in raw]
    for line in lines:
        if not line.startswith("# "):
            raise ValueError(f"AI header line must start with '# ': {line!r}")
    return lines
