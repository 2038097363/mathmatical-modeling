...
# 模块3：只采用一对一、周期等价且共线的端点接头；多解必须显式保留。
...
# 模块4：按已选接头形成潜在粒子链，但绝不补齐缺失长度或延伸短段。
...
# 模块6：从原始附件一次重建全部声明场景，并校验行分区与长度守恒。
...
# 模块7：把派生映射和审计报告写入 processed，原始附件始终只读。
