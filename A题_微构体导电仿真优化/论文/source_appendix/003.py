...
from reconstruct_segments import reconstruct_workbook, write_processed_outputs  # noqa: E402
from result_registry import (  # noqa: E402
    DEFAULT_REGISTRY,
    DEFAULT_TEX,
    export_latex,
    read_registry,
...
# 模块1：读取附件行并构造原尺寸有限平底圆柱，任何短段都不延长或补齐。
...
# 模块2：用独立六变量凸约束优化复核 GJK 的实际窄相阈值分类。
...
# 模块3：胶囊宽相只负责排除；所有候选均进入平底圆柱 GJK 并输出距离界。
...
# 模块4：只把重建器确认的一对一周期接头作为同一粒子的可切换内部边。
...
# 模块5：并查集给出贯通判定；路径搜索只负责恢复可核查的逐边见证。
...
def shortest_path(edges: Iterable[dict[str, Any]]) -> tuple[list[str] | None, list[dict[str, Any]]]:
    ...
    while nodes[-1] != "LEFT":
        previous, edge = predecessor[nodes[-1]]  # type: ignore[misc]
...
# 模块6：从只读附件重建身份、计算全部物理边，并枚举三种解释与两种内部语义。
...
def register_q1_results(
    ...
    *,
    ...
    # Remove every prior Q1 entry first. This also clears unknown legacy keys
    # and makes repeated runs converge to the same official key set.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Q1 逐行独立正式结论与历史重联诊断的有限平底圆柱审计"
    )
