...
# 稳定整数 SeedSequence 分流生成 IID 中心与各向同性方向。
# 关键：按试验编号派生稳定随机流，保证并行运行可复现。
...
def _witness_path(
    ...
    while nodes[-1] != left_node:
        previous, edge_type = predecessor[nodes[-1]]
...
# 按原粒子前缀加入全部片段，并以并查集判断首次贯通时刻。
# 关键：扫描共同随机前缀并返回首次电极连通的粒子数。
...
# 关键：计算二项分布 Clopper-Pearson 精确置信区间。
