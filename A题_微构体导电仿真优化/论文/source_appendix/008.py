...
except ImportError:  # pragma: no cover - the project pins numba at runtime.
...
class FastDistanceResult:
    """Distance bounds plus an audit trail for a possible reference fallback."""
...
class FastClassificationResult:
    """Strict threshold decision with distance- and decision-convergence split."""
...
def _project_origin_to_simplex(
    ...
    """Project onto a <=5 point hull and compact its active <=4 point simplex."""
    ...
    # Active indices are increasing, so forward compaction cannot overwrite a
    # source point needed by a later copy.
...
def fast_cylinder_distance_diagnostics(
    ...
    *,
    ...
    """Return conservative cylinder bounds, falling back on any risky case.

    Supplying ``cutoff`` enables the strict threshold path. Results within
    ``threshold_guard`` of that cutoff are recomputed by the reference kernel.
    """
...
def fast_cylinder_distance_bounds(
    ...
    *,
    ...
    """Drop-in cylinder-only bounds API; pass ``cutoff`` for strict use."""
...
def fast_cylinder_classify(
    ...
    *,
    ...
    """Classify a cylinder pair using certified bounds and guarded fallback."""
...
def warm_up_fast_geometry() -> None:
    """Compile/load the Numba kernel before starting timed or worker code."""
    if not NUMBA_AVAILABLE:
        return
    first = Cylinder(
