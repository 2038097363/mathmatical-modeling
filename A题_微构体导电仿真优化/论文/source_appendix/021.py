...
def _short_node_label(node: str) -> str:
    if node == "electrode_left":
        return "左电极"
    if node == "electrode_right":
        return "右电极"
    parts = node.split("_")
    return f"{parts[0]}源{int(parts[1][1:])} / 片{int(parts[2][1:])}"


def _draw_contact_chain(ax: Any, scene: dict[str, Any], edges: list[dict[str, Any]]) -> None:
    nodes = scene["traceability"]["mixed_witness"]["nodes"]
    y_values = np.linspace(0.96, 0.04, len(nodes))
    ax.set_xlim(0.0, 1.0)
    ax.set_ylim(0.0, 1.0)
    ax.axis("off")
    for index in range(len(nodes) - 1):
        ...
        ax.text(
            0.29,
            0.5 * (y_values[index] + y_values[index + 1]),
            "电极-A" if edges[index]["contact_type"] == "electrode-A" else "A-A",
            va="center",
            fontsize=7.2,
            color="#46515C",
        )
    for index, node in enumerate(nodes):
        ...
        ax.text(0.48, y_values[index], _short_node_label(node), va="center", fontsize=8.0)
    ax.set_title(
        f"有序接触图（{len(edges)} 条边）", fontsize=11, fontweight="bold", pad=8
    )
...
def _pixel_audit(path: Path, expected_topology_edges: int) -> dict[str, Any]:
    ...
    left_width = int(0.70 * rgb.shape[1])
...
def build_figure(
    ...
    figure = plt.figure(figsize=(11.6, 6.5), facecolor="white")
    grid = figure.add_gridspec(1, 2, width_ratios=(2.35, 1.0), wspace=0.06)
    ...
    _draw_contact_chain(chain_ax, scene, edges)
    legend = [
        Line2D([0], [0], color=COLOR_A, lw=3, label=f"介质 A 总数：{int(design['n_a'])}（背景见总览）"),
        Line2D([0], [0], marker="o", color="none", markeredgecolor=COLOR_B, label=f"介质 B：N_B={int(design['n_b'])}（未出现）"),
        Line2D([0], [0], color=COLOR_WITNESS, lw=4, label=f"见证片段轴线：{len(records)} 个"),
        Line2D([0], [0], color=COLOR_TOPOLOGY, lw=2.3, linestyle=(0, (5, 2)), label=f"见证图边：{len(edges)} 条"),
        Line2D([0], [0], color=COLOR_ELECTRODE, lw=2, label="左右电极"),
        Line2D([0], [0], color=COLOR_BOUNDARY, lw=1, linestyle=(0, (3, 2)), label="局部显示窗"),
    ]
    figure.legend(
        handles=legend,
        loc="lower center",
    ...
    figure.subplots_adjust(left=0.04, right=0.98, top=0.93, bottom=0.14)
