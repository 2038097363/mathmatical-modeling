...
# 模块3：Gilbert/GJK 在 Minkowski 差集上迭代，并保留可行上界与支持平面下界。
...
# 模块5：胶囊距离仅作平底圆柱距离的安全下界宽相，不承担最终接触判定。
...
# 模块6：显式环面最小镜像只供明确周期域使用，Q1 附件图禁止全局调用。
