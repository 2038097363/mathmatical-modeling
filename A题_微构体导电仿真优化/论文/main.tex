\documentclass[UTF8,zihao=-4,fontset=none,a4paper]{ctexart}

\usepackage{amsmath,amssymb,bm,mathtools}
\usepackage{booktabs,array,tabularx,longtable,multirow}
\usepackage{graphicx,float,caption,subcaption,placeins}
\usepackage{geometry,xcolor,hyperref,fancyhdr}
\usepackage{enumitem,xurl}
\usepackage{fvextra}
\usepackage{needspace}
\newcounter{none}

\geometry{left=2.5cm,right=2.5cm,top=2.5cm,bottom=2.5cm,headheight=0pt,headsep=0pt,footskip=0.75cm}
\graphicspath{{./}}
\IfFontExistsTF{Times New Roman}
  {\setmainfont{Times New Roman}}
  {\setmainfont{TeX Gyre Termes}}
\IfFontExistsTF{Menlo}
  {\setmonofont{Menlo}}
  {\IfFontExistsTF{Consolas}
    {\setmonofont{Consolas}}
    {\setmonofont{Latin Modern Mono}}}

% 优先探测当前平台常用字体，避免 XeLaTeX 为缺失字体启动无效的字体生成流程。
\IfFontExistsTF{Songti SC}{
  \setCJKmainfont{Songti SC}[BoldFont=Heiti SC]
  \setCJKsansfont{Heiti SC}[AutoFakeBold=2]
  \setCJKmonofont{Songti SC}[AutoFakeBold=2]
  \setCJKfamilyfont{mcmsong}{Songti SC}[BoldFont=Heiti SC]
  \setCJKfamilyfont{mcmhei}{Heiti SC}[AutoFakeBold=2]
  \setCJKfamilyfont{mcmfang}{Songti SC}[AutoFakeBold=2]
}{
  \IfFontExistsTF{SimSun}{
    \setCJKmainfont{SimSun}[BoldFont=SimHei]
    \setCJKsansfont{SimHei}[AutoFakeBold=2]
    \setCJKmonofont{FangSong}[AutoFakeBold=2]
    \setCJKfamilyfont{mcmsong}{SimSun}[BoldFont=SimHei]
    \setCJKfamilyfont{mcmhei}{SimHei}[AutoFakeBold=2]
    \setCJKfamilyfont{mcmfang}{FangSong}[AutoFakeBold=2]
  }{
    \setCJKmainfont{FandolSong-Regular}[BoldFont=FandolSong-Bold]
    \setCJKsansfont{FandolHei-Regular}[AutoFakeBold=2]
    \setCJKmonofont{FandolFang-Regular}[AutoFakeBold=2]
    \setCJKfamilyfont{mcmsong}{FandolSong-Regular}[BoldFont=FandolSong-Bold]
    \setCJKfamilyfont{mcmhei}{FandolHei-Regular}[AutoFakeBold=2]
    \setCJKfamilyfont{mcmfang}{FandolFang-Regular}[AutoFakeBold=2]
  }
}
\providecommand{\songti}{}
\providecommand{\heiti}{}
\providecommand{\fangsong}{}
\renewcommand{\songti}{\CJKfamily{mcmsong}}
\renewcommand{\heiti}{\CJKfamily{mcmhei}}
\renewcommand{\fangsong}{\CJKfamily{mcmfang}}
\setlength{\parindent}{2em}
\setlength{\parskip}{0pt}
\setlength{\emergencystretch}{3em}
\linespread{1.0}\selectfont
\renewcommand{\arraystretch}{1.12}
\setlength{\tabcolsep}{5pt}
\setlist{nosep,leftmargin=2.3em}
\setcounter{secnumdepth}{0}

\ctexset{
  section={format=\centering\heiti\zihao{4},beforeskip=0.9em,afterskip=0.55em},
  subsection={format=\raggedright\heiti\zihao{-4},beforeskip=0.65em,afterskip=0.35em},
  subsubsection={format=\raggedright\songti\bfseries\zihao{-4},beforeskip=0.45em,afterskip=0.25em}
}

\captionsetup{font=small,labelsep=quad,skip=4pt}
\setlength{\floatsep}{8pt plus 2pt minus 2pt}
\setlength{\textfloatsep}{9pt plus 2pt minus 2pt}
\setlength{\intextsep}{7pt plus 1pt minus 1pt}
\renewcommand{\topfraction}{0.92}
\renewcommand{\bottomfraction}{0.82}
\renewcommand{\textfraction}{0.06}
\renewcommand{\floatpagefraction}{0.78}
\setcounter{topnumber}{3}
\setcounter{bottomnumber}{2}
\setcounter{totalnumber}{4}

\definecolor{mainblue}{RGB}{25,91,132}
\definecolor{safe}{RGB}{0,122,112}
\definecolor{danger}{RGB}{203,66,53}
\hypersetup{
  hidelinks,
  pdfauthor={},
  pdftitle={基于随机几何接触图与精确置信界的微构体导电优化},
  pdfsubject={},
  pdfkeywords={}
}
\urlstyle{same}

\pagestyle{fancy}
\fancyhf{}
\fancyfoot[C]{\thepage}
\renewcommand{\headrulewidth}{0pt}
\renewcommand{\footrulewidth}{0pt}

\newcommand{\CoverTopCell}[1]{\parbox[c][0.55cm][c]{\linewidth}{\centering\songti\zihao{5} #1}}
\newcommand{\CoverBottomCell}[1]{\parbox[c][1.34cm][c]{\linewidth}{\centering\songti\zihao{5} #1}}
\newcommand{\TemplateMatterHeading}[1]{%
  \par\addvspace{0.8em}%
  {\noindent\songti\bfseries\zihao{-4} #1\par}%
  \vspace{0.35em}%
}
\renewenvironment{abstract}{\par}{\par}
\providecommand{\tightlist}{%
  \setlength{\itemsep}{0pt}\setlength{\parskip}{0pt}}
\InputIfFileExists{build_meta.tex}{}{%
  \PackageError{paper}{Missing build_meta.tex}{Keep build_meta.tex beside main.tex}}

\begin{document}
\pagenumbering{arabic}
\thispagestyle{fancy}

\noindent
\begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}p{0.167\textwidth}|>{\centering\arraybackslash}X|>{\centering\arraybackslash}p{0.15\textwidth}|}
\hline
\CoverTopCell{所属类别} & \multirow{2}{=}{\centering\heiti\zihao{4}\bfseries 2026 年“华数杯”大学生数学建模竞赛} & \CoverTopCell{参赛编号} \\
\cline{1-1}\cline{3-3}
\CoverBottomCell{\PaperGroup} & & \CoverBottomCell{\PaperCompetitionId} \\
\hline
\end{tabularx}

\PaperBuildNotice

\vspace{0.45cm}
\begin{center}
  {\heiti\fontsize{15pt}{18pt}\selectfont\bfseries 基于随机几何接触图与精确置信界的微构体导电优化\par}
  \vspace{0.65cm}
  {\heiti\zihao{4}\bfseries 摘\quad{}要\par}
\end{center}

\vspace{0.25cm}
\begin{abstract}
在随机填充的微构体中，增加导电介质会提高形成贯通通路的可能性，同时增加材料成本，因此需要同时考虑导通概率和填充成本。介质形状、随机位置以及边界截断都会改变局部接触关系，使相同填充量下的微构体具有不同的连通状态。针对题给的圆柱和球形导电介质，本文将截断后的片段作为独立导电单元，以 \(1.8\,\mathrm{nm}\) 为接触判据构建三维接触网络，并通过实体间距离计算判断两侧电极是否连通。在这一统一模型上，进一步采用蒙特卡洛（Monte Carlo）模拟估计导通概率，并研究满足 \(90\%\) 导通概率要求时的最低填充量和低成本介质配比。

\textbf{针对问题一，}三组附件微构体的结论依次为\textbf{不导通、导通、导通}；$144087$ 个粒子对中仅 $195$ 对进入凸体窄相判定，且与独立约束优化校验的阈值分类完全一致。

\textbf{针对问题二，}使用固定随机流的共同粒子前缀和 $20000$ 次蒙特卡洛试验，得到 $0.50\%$、$0.60\%$、$0.70\%$ 和 $1.00\%$ 填充量下的导通概率分别为 $\mathbf{7.895\%}$、$\mathbf{22.050\%}$、$\mathbf{47.195\%}$ 和 $\mathbf{99.280\%}$，Wilson 区间与分块复算表明贯通跃迁位置稳定。

\textbf{针对问题三，}对全 A 临界整数使用 $100000$ 个统一样本，并以 Bonferroni--Clopper--Pearson 联合精确界判定可行性。615 根的经验导通率为 $\mathbf{90.403\%}$，校正单侧下限为 $\mathbf{90.1176\%}$；因此，$\mathbf{615}$ 根是最小可行整数，对应最优填充量 $\mathbf{0.87\%}$。

\PaperQFourAbstract

统一计算链由有限凸几何生成接触图，以共同前缀估计导通概率，并在预先确定的低成本候选域及全 A 参照上实施联合精确判定。几何距离界、概率区间与成本约束使用同一接触阈值和边界语义，从而直接连接确定性导通、概率跃迁与候选域内成本排序。
\end{abstract}

\noindent\textbf{关键词：}随机几何图；连通渗流；GJK 距离；Monte Carlo 仿真；Clopper--Pearson 精确界；概率约束优化

\clearpage
\input{content.tex}

\end{document}
